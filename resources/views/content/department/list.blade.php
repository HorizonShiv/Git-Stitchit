...
@section('content')
    <h4 class="py-3 mb-4">
        <span class="text-muted fw-light">Master / Department /</span> List
    </h4>
    ...
    <div class="card">
        <div class="card-datatable table-responsive pt-0">
            <table class="datatables-basic table" id="datatable-list">
                ...
                <tbody>
                    @php $num = 1 @endphp
                    @foreach (\App\Models\Department::with('ProcessMaster')->get() as $Department)
                        <tr>
                            <td class="text-bold"><a href="">{{ $num }}</a></td>
                            <td>{{ $Department->name }}</td>
                            <td>{{ $Department->ProcessMaster->name ?? ''}}</td>
                            <td>
