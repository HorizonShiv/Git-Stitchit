...
@section('content')
    <h4 class="py-3 mb-4">
        <span class="text-muted fw-light">Warehouse / GR /</span> List
    </h4>
    ...
    <div class="card">
        <div class="card-datatable table-responsive pt-0">
            <table class="datatables-basic table" id="datatable-list">
                ...
                <tbody>
                    @php $num = 1 @endphp
                    @foreach ($GrToSuppliers as $GrToSupplier)
                        <tr>
                            <td class="text-bold"><a href="">{{ $num }}</a></td>
                            <td>
                                {{ $GrToSupplier->grn_no }}
                            </td>
                            <td>{{ $GrToSupplier->WareHouse->name }}</td>
                            <td>
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="card-info">
                                        <h5 class="mb-0">{{ $GrToSupplier->User->person_name }}</h5>
                                        <small>{{ $GrToSupplier->User->company_name }}</small>
                                    </div>
                                </div>
                            </td>
                            <td>
                                <ul style="list-style-type:none;">
                                    @foreach ($GrToSupplier->GrToSupplierParameter as $GrToSupplierParameter)
                                        <li> {{ $GrToSupplierParameter->Item->name }}</li>
                                    @endforeach
                                </ul>
                            </td>
                            <td>
                                <ul style="list-style-type:none;">
                                    @foreach ($GrToSupplier->GrToSupplierParameter as $GrToSupplierParameter)
                                        <li>
                                            {{ $GrToSupplierParameter->return_qty }}</li>
                                    @endforeach
                                </ul>
                            </td>
                            <td>
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="card-info">
                                        <h5 class="mb-0">{{ $GrToSupplier->UserInfo->person_name }}</h5>
                                        <small>{{ $GrToSupplier->UserInfo->company_name }}</small>
                                    </div>
                                </div>
                            </td>
                            <td>
